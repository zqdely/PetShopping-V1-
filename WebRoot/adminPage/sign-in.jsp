...
<script type="text/javascript">
	 ...
	 function checkNumber(){
		 ...
		 if( number==""){
		     $ninfo.html("库存数量不能为空!"); 	
			 return false;
	     }else if(isnumber.test(number)==false){
		     $ninfo.html("库存数量必须为合法数字(整数)!");  	 
			 return false; 	 
		 }else{
	         $ninfo.html("<img src='../adminPage/images/ok.png'/>"); 	 
			 return true; 
		 }
	 };
</script>
				...
				<div class="well">
					...
					<div id="myTabContent" class="tab-content">
						...
						<form action="uploadFile.action" enctype="multipart/form-data"
								method="post">
								<label>商品名称</label> 
								<input type="text" name="products.name" id="name" onblur="checkName();" required="required"> 
								<span id="nainfo"></span> 
								<label>商品价格</label> 
								<input type="text"name="products.price" id="price" onblur="checkPrice();" required="required">&nbsp;&nbsp;
								<span id="pinfo"></span> 
								<label>商品图片</label>
								...
								<label>商品简要信息</label>
								<input type="text" name="products.intro" id="iinfo" onblur="checkInfo();" required="required">
								<span id="info"></span>
								<label>商品详细信息</label>
								<textarea name="products.info" value="Smith" rows="2"
									class="input-xlarge" style="width: 206px;"> 
                    
                                </textarea>
								<label>商品种类</label> <select name="protypeid">
									<option value="0" selected="selected">请选择商品种类</option>
									<%
								...
								<label>库存数量</label> 
								<input type="text" name="products.procount" id="number" onblur="checkNumber();" required="required">
								<span id="ninfo"></span><br/> 
