...
<div class="well">
    ...
    <form id="tab" action="update" method="post">
        <label>&nbsp;用户名</label>
        <s:textfield name="user.name" class="input-xlarge" readonly="true"></s:textfield>
        <label>&nbsp;邮箱</label>
        <s:textfield name="user.email" class="input-xlarge" readonly="true"></s:textfield>
        <label>&nbsp;地址</label>
        ...
        <s:hidden name="user.sex"></s:hidden>
        <s:hidden name="user.isadmin"></s:hidden>
        <s:hidden name="user.activation"></s:hidden>
        <s:submit style="float:left; " value="修改信息"></s:submit>
    </form>
      </div>
      <div class="tab-pane fade" id="profile">
    <form id="tab2">
        <div>
            <a href="<%=path %>/ResetPassword.jsp" class="btn btn-primary">修改密码</a><span style="color:red; font-size:12px;">只能通过邮箱修改密码</span>
        </div>
    </form>
      </div>
  </div>
